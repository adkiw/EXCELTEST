...
# 1) LT savaitės dienos pilnu pavadinimu
lt_weekdays = {
    0: "Pirmadienis",
    1: "Antradienis",
    2: "Trečiadienis",
...
# 2) Funkcija – Excel raidė stulpeliui
...
# 3) Pagalbinė – pirmadienis duotai datai
def iso_monday(d: date) -> date:
    return d - timedelta(days=(d.isoweekday() - 1))

# 4) Numatyti intervalai (−2 … +2 savaitės)
today = date.today()
...
# 14) Apskaičiuojam stulpelių skaičių
total_common   = len(common_headers)
total_day_cols = len(dates) * len(day_headers)
# +1 už pirmąjį stulpelį “#”
total_all_cols = 1 + total_common + total_day_cols

# 15) Generuojam HTML
html = '<div class="table-container"><table>\n'

# 15.1) Raidinis header’as
html += "<tr>"
for i in range(1, total_all_cols+1):
    html += f"<th>{col_letter(i)}</th>"
html += "</tr>\n"

# 15.2) Datų eilutė su data + pilna savaitės diena
html += "<tr>"
# pirmas stulpelis po raidės A lieka tuščias
html += "<th></th>"
# tuščias block virš common+Pastabos
